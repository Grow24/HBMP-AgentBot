...
let randomState;
// Lazy load randomState from openid-client to avoid ESM errors when OpenID is not configured
try {
  ...
  if (err.code === 'ERR_REQUIRE_ESM') {
    console.warn('[oauth.js] openid-client is ES module, will use dynamic import when needed');
  } else {
    throw err;
  }
}
...
router.get('/openid', async (req, res, next) => {
  // Dynamic import for randomState since openid-client is now an ES module
  if (!randomState) {
    const openidClientModule = await import('openid-client');
    randomState = openidClientModule.randomState || (openidClientModule.default && openidClientModule.default.randomState);
  }
