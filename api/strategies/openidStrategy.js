...
let client;
let OpenIDStrategy;
// Lazy load openid-client to avoid ESM errors when OpenID is not configured
try {
  client = require('openid-client');
  OpenIDStrategy = require('openid-client/passport').Strategy;
} catch (err) {
  // openid-client is an ES module and cannot be required in CommonJS
  // Will use dynamic import when OpenID is actually needed
  if (err.code === 'ERR_REQUIRE_ESM') {
    console.warn('[openidStrategy] openid-client is ES module, will use dynamic import when needed');
  } else {
    throw err;
  }
}
...
async function getCustomOpenIDStrategy() {
  if (!CustomOpenIDStrategy) {
    // Dynamic import for openid-client/passport since it's now an ES module
    if (!OpenIDStrategy) {
      const passportModule = await import('openid-client/passport');
      OpenIDStrategy = passportModule.Strategy || (passportModule.default && passportModule.default.Strategy);
    }
...
async function setupOpenId() {
  try {
    // Dynamic import for openid-client since it's now an ES module
    if (!client) {
      const openidClientModule = await import('openid-client');
      client = openidClientModule.default || openidClientModule;
    }
    ...
    const OpenIDStrategyClass = await getCustomOpenIDStrategy();
    const openidLogin = new OpenIDStrategyClass(
      {
        config: openidConfig,
